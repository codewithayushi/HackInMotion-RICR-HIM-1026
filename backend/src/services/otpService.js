...
// User Provided Credentials Default Configuration
...
/**
 * Creates SMTP Nodemailer Transporter
 */
...
/**
 * Sends 6-digit OTP via SMS provider (Fast2SMS)
 */
const sendSMS = async (phone, otp) => {
  ...
  try {
    ...
    if (responseData && responseData.status_code === 999) {
      console.warn(`[Fast2SMS Account Notice] ${responseData.message} (Note: Fast2SMS requires 100 INR recharge for live carrier SMS API routes).`);
    } else {
...
/**
 * Sends professional HTML Email OTP via Nodemailer SMTP
 */
const sendEmail = async (email, otp, purpose) => {
    ...
    <div style="font-family: 'Inter', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
      ...
      <div style="padding: 32px 24px;">
        ...
        <div style="background-color: #f8fafc; border: 2px dashed #2563eb; border-radius: 12px; padding: 20px; text-align: center; margin: 24px 0;">
          <span style="font-size: 12px; color: #64748b; text-transform: uppercase; font-weight: 700; letter-spacing: 1px; display: block; margin-bottom: 8px;">Your 6-Digit OTP Code</span>
        ...
        <p style="color: #64748b; font-size: 13px; line-height: 1.5;">
          ...
          🔒 <strong>Security Alert:</strong> Never share this OTP code with anyone.
        </p>
      </div>
  ...
  try {
    ...
    console.log(`[Email Service Success] Message sent to ${email}: ${info.messageId}`);
